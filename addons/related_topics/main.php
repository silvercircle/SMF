 ...
 * @version 1.0pre
 *
 * Related topics plugin, based on the original related topics mod (1.4)
 * for SMF, written by nikop.
 *
 * This is a testcase and WIP to verify the usability of the EoS plugin
 * system for a bigger modification doable without code edits.
 */

function related_topics_autoloader()
{
	return new RelatedTopics();
}

class RelatedTopics extends EoS_Plugin
{
	protected $productShortName = 'related_topics';
	protected $installableHooks = array(
		'display_general' => array('file' => 'main.php', 'callable' => 'RelatedTopics::load')
	);

	protected $_product = array(
		'Version' => '0.1',
		'Name' => 'Related Topics',
		'Description' => 'A plugin to implement related (or "similar") topics in topic display.'
	);

	public function __construct() { parent::__construct(); }

	/**
	 * @param $topic int - 	topic id for which we want to load the related
	 * 						topics.
	 * @return bool			false if there are no related topics.
	 *
	 * load related topics for a given topic id. This runs once in Display()
	 */
	public static function load($topic)
	{
		...
		EoS_Smarty::getConfigInstance()->registerHookTemplate('topic_after', 'related_topics_output');

		return self::prepareTopicArray($topics_ids);
	}

	/**
	 * @param $topic_ids array of topic ids which are considered related to the original
	 * 						   topic.
	 * @return bool
	 *
	 * TODO make this use the standard Topiclist class.
	 */
	public static function prepareTopicArray($topic_ids)
	{
		global $scripturl, $txt, $modSettings, $context, $user_info, $settings;
