...
function loadBoard()
{
	...
	if (empty($temp))
	{
		$request = smf_db_query( '
			...
			array(
				'current_topic' => $topic,
				'board_link' => empty($topic) ? smf_db_quote('{int:current_board}', array('current_board' => $board)) : 't.id_board',
			)
		);
		...
		if (mysql_num_rows($request) > 0)
		{
			...
			{
				mysql_free_result($request); // Free the previous result
...
function loadMemberContext($user, $display_custom_fields = false)
{
	...
	$memberContext[$user] = array(
		...
		'link' => '<a href="' . $scripturl . '?action=profile;u=' . $profile['id_member'] . '" title="' . $txt['profile_of'] . ' ' . $profile['real_name'] . '">' . $profile['real_name'] . '</a><span onclick="getMcard('.$profile['id_member'].', $(this));return(false);" class="mcard">&nbsp;&nbsp;</span>',
