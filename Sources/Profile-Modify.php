...
function authentication($memID, $saving = false)
{
	...
	if ($saving)
	{
		...
		if ($_POST['authenticate'] == 'passwd')
		{
			...
			{
				// Integration?
				HookAPI::callHook('integrate_reset_pass', array($cur_profile['member_name'], $cur_profile['member_name'], $_POST['passwrd1']));
...
function list_getTopicNotificationCount($memID)
{
	...
	list ($totalNotifications) = mysql_fetch_row($request);
	mysql_free_result($request);
