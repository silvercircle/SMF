...
function EditCategory()
{
	...
	else
	{
		$context['category'] = array(
			...
			'desc' => htmlspecialchars($cat_tree[$_REQUEST['cat']]['node']['desc']),
...
function EditBoard2()
{
	...
	if (isset($_POST['edit']) || isset($_POST['add']))
	{
		...
		$boardOptions['is_pageboard'] = isset($_POST['is_pageboard']) ? 1 : 0;
...
function EditBoardSettings($return_config = false)
{
	...
	$config_vars = array(
		...
		'',
			array('int', 'tidy_child_display_columns'),
			array('int', 'child_board_desc_shortened'),
	);
