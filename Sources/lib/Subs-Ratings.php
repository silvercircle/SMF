		...
		if(is_array($like_status)) {
			foreach($like_status as $key => $the_like) {
				if(isset($modSettings['ratings'][$key]) && isset($the_like['members'])) {
					...
					if($the_like['count'] > 1)
						$parts[$key] .= (in_array($key, $types) ? sprintf($the_like['count'] > 2 ? $txt['you_and_others'] : $txt['you_and_other'], $the_like['count'] - 1) : '(<a rel="nofollow" attr-mid="'.$the_like['members'][0]['id'].'" class="member mcard" href="'.URL::user($the_like['members'][0]['id'], $the_like['members'][0]['name']) .'">'.$the_like['members'][0]['name'].'</a>&nbsp;' . sprintf($the_like['count'] > 2 ? $txt['and_others'] : $txt['and_other'], $the_like['count'] - 1));
					else
						$parts[$key] .= (in_array($key, $types) ? $txt['rated_you'] : '(<a rel="nofollow" attr-mid="'.$the_like['members'][0]['id'].'" class="member mcard" href="'.URL::user($the_like['members'][0]['id'], $the_like['members'][0]['name']) .'">'.$the_like['members'][0]['name'].'</a>)');
				}
			}
		}
