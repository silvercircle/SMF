...
function MessageFolder()
{
	...
	if ($context['display_mode'] == 2)
	{
		...
		if ($smcFunc['db_title'] == 'PostgreSQL' && $_GET['sort'] != 'pm.id_pm')
		{
			...
			$sub_pms = array();
			while ($row = mysql_fetch_assoc($sub_request))
				$sub_pms[$row['id_pm_head']] = $row['sort_param'];

			mysql_free_result($sub_request);
...
function prepareMessageContext($type = 'subject', $reset = false)
{
	...
	{
		$subject = mysql_fetch_assoc($subjects_request);
		if (!$subject)
		{
			mysql_free_result($subjects_request);
			return false;
		}
	...
	if ($context['folder'] == 'inbox' && !empty($search_params['advanced']) && $context['currently_using_labels'])
	{
		...
		elseif (count($_REQUEST['searchlabel']) != count($context['labels']))
		{
			...
			foreach ($_REQUEST['searchlabel'] as $label)
				$labelStatements[] = smf_db_quote('FIND_IN_SET({string:label}, pmr.labels) != 0', array(
					'label' => $label,
				));
...
function MessagePost()
{
	...
	if (!empty($_REQUEST['pmsg']))
	{
		...
		$isReceived = mysql_num_rows($request) != 0;
		mysql_free_result($request);
...
function isAccessiblePM($pmID, $validFor = 'in_or_outbox')
{
	...
	$validationResult = mysql_fetch_assoc($request);
	mysql_free_result($request);
