...
class EoS_Twig {
	...
	private static $_custom_functions = array();
	...
	public static function init()
	{
		...
		if(file_exists($settings['theme_dir'] . '/twig_theme_support.php')) {
			@require_once($settings['theme_dir'] . '/twig_theme_support.php');
	 ...
	 * 
	 * allows adding of additional functions that are only used by certain
	 * modules (i.e. Display.php uses this to make the message callback function
	 * available to the templates)
	 */
	public static function addFunction($name = '', $callable = '')
	{
		if(!empty($name) && !empty($callable) && is_callable($callable))
			self::$_custom_functions[$name] = $callable;
	}
	/**
	...
	public static function Display()
	{
		...
		$_fns = self::$_theme_support->getFunctions();
  		...
  		if(!empty(self::$_custom_functions)) {
  			foreach(self::$_custom_functions as $name => $callable)
  				self::$_twig_environment->addFunction($name, new Twig_Function_Function($callable));
  		}
