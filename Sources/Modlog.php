...
function ViewModlog()
{
	...
	$listOptions = array(
		...
		'title' => '<a href="' . $scripturl . '?action=helpadmin;help=' . ($context['log_type'] == 3 ? 'adminlog' : 'modlog') . '" onclick="return reqWin(this.href);" class="help"><img class="icon" src="' . $settings['images_url'] . '/helptopics.png" alt="' . $txt['help'] . '" /></a> ' . $txt['modlog_' . ($context['log_type'] == 3 ? 'admin' : 'moderation') . '_log'],
		'width' => '100%',
...
function list_getModLogEntryCount($query_string = '', $query_params = array(), $log_type = 1)
{
	...
	list ($entry_count) = mysql_fetch_row($result);
