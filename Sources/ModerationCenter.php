...
function ModBlockNotes()
{
	...
	if (isset($_POST['makenote']) && isset($_POST['new_note']))
	{
		...
		$_POST['new_note'] = commonAPI::htmlspecialchars(trim($_POST['new_note']));
...
function ModReport()
{
	...
	if (isset($_POST['add_comment']) && !empty($_POST['mod_comment']))
	{
		...
		$newComment = trim(commonAPI::htmlspecialchars($_POST['mod_comment']));
...
function ShowNotice()
{
	...
	if (mysql_num_rows($request) == 0)
		fatal_lang_error('no_access', false);
	list ($context['notice_body'], $context['notice_subject']) = mysql_fetch_row($request);
	mysql_free_result($request);
...
function list_getWatchedUserPostsCount($approve_query)
{
	...
	list ($totalMemberPosts) = mysql_fetch_row($request);
	mysql_free_result($request);
...
function list_getWarningCount()
{
	...
	list ($totalWarns) = mysql_fetch_row($request);
	mysql_free_result($request);
...
function list_getWarningTemplates($start, $items_per_page, $sort)
{
	...
	while ($row = mysql_fetch_assoc($request))
	{
		$templates[] = array(
			...
			'body' => commonAPI::htmlspecialchars($row['body']),
		);
	}
	mysql_free_result($request);
