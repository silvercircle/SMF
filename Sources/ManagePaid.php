...
function ModifySubscription()
{
	...
	if (isset($_POST['save']))
	{
		...
		else
		{
			...
			list ($disableGroups) = mysql_fetch_row($request);
	...
	else
	{
		...
		list ($context['disable_groups']) = mysql_fetch_row($request);
		mysql_free_result($request);
	}
...
function ModifyUserSubscription()
{
	...
	if ($context['log_id'] && !$context['sub_id'])
	{
		...
		if (mysql_num_rows($request) == 0)
			fatal_lang_error('no_access', false);
		list ($context['sub_id']) = mysql_fetch_row($request);
		mysql_free_result($request);
	}
	...
	elseif (isset($_REQUEST['save_sub']))
	{
		...
		if (empty($context['log_id']))
		{
			...
			list ($id_member, $id_group) = mysql_fetch_row($request);
			mysql_free_result($request);
		...
		else
		{
			...
			list ($id_member, $old_status) = mysql_fetch_row($request);
			mysql_free_result($request);
	...
	if ($context['action_type'] == 'add')
	{
		...
		if (isset($_GET['uid']))
		{
			...
			list ($context['sub']['username']) = mysql_fetch_row($request);
			mysql_free_result($request);
		}
		else
...
function removeSubscription($id_subscribe, $id_member, $delete = false)
{
	...
	list ($id_group, $additional_groups) = mysql_fetch_row($request);
	mysql_free_result($request);
