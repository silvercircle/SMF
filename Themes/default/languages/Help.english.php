...
$helptxt['legacyBBC'] = 'Selecting this option will enable support for the old SMF 2 compatible BBCodes that were removed from Eos Alpha';
...
$helptxt['enableUserTagging'] = 'If enabled, members can mention other members in posts, using a simple @Name1,name2: syntax.<br><br>
	Mentioned members are converted to user profile links and if the activity stream is enabled, they\'ll be sent a notification
	with a link to the post that contains their "tagged" username. So basically, this feature is only useful when you are using the activity stream on your forum.<br><br>
	<strong>Note: </strong>This setting is only a global switch to enable or disable the feature board-wide. In order to use it,
	you must also grant proper permissions to user groups that should be allowed to use it. You\'ll find the permission in the list of board permissions under <em>Posts->Tag (@mention) users in posts</em>.';
...
$helptxt['embed_piwik'] = 'If checked, the code needed to use the piwik web analytics system will be embedded in the footer of every page. You absolutely <b>must</b> configure the URL and tracker id below.<br><br>Note that registered members can opt out from being tracked to protect their privacy.';
$helptxt['piwik_uri'] = 'The <b>full</b> URL to the piwik installation you want to use. For example, if you have piwik on http://mydomain.com/piwik, this is exactly what you need to enter here. <b>If your forum is hosted on https, make sure to have piwik on https as well, otherwise your visitors will get security warnings!!</b>';
$helptxt['piwik_tracker_id'] = 'This is the <b>numeric</b> value of the tracker ID you want to use. A single piwik installation can be used for tracking different sites, so make sure you\'re using the correct tracker id here';
...
$helptxt['linkSecurity'] = 'When enabled, an additional permission will control how links posted by members will behave. Links posted by members with insufficient permissions, using the <strong>[URL]</strong> BBC tag will not be followed directly. Instead, a page will explain why the link was not followed and present the target URL in plain text format (non-clickable).
	<br>
	<br>
