  ...
  {if empty($board.is_page)}
    ...
    <div style="padding-left:32px;">
      <h3 style="display:inline;">
        <a class="boardlink" data-tip="tip_b_{$board.id}" href="{$board.href}" id="b{$board.id}">{$board.name}</a>
      </h3>
      ...
      {/if}
      <span class="floatright smalltext">{$board.posts} <span class="lowcontrast">{$T.posts} {$T.in}</span> {$board.topics} <span class="lowcontrast">{$T.topics}</span></span>
      <div class="clear tinytext lowcontrast">
      {if !empty($board.last_post.id)}
        {$board.last_post.prefix}{$board.last_post.topiclink}
        <a title="{$T.last_post}" href="{$board.last_post.href}">{$board.last_post.time}</a>
        <span class="tinytext lowcontrast">{$T.last_post}&nbsp;{$T.by}&nbsp;</span>{$board.last_post.member.link}
      {else}
        {$T.not_applicable}
      {/if}
    </div>
...
{function boardbit_subcat}
  <li id="board_'{$board.id}" class="subcatrow">
    ...
    {if !empty($board.last_post.id)}
      <div class="tinytext nowrap righttext">
      <a title="{$T.last_post}" href="{$board.last_post.href}">{$board.last_post.time}</a><span class="tinytext lowcontrast">{$T.last_post} in: </span>{$board.last_post.prefix}{$board.last_post.topiclink}
